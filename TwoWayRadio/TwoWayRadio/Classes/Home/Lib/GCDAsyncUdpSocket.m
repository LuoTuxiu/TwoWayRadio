...
@class GCDAsyncUdpSendPacket;//使用到了前向引用？
...
@interface GCDAsyncUdpSocket ()
{
	...
	GCDAsyncUdpSocketReceiveFilterBlock receiveFilterBlock;//过滤器？
	dispatch_queue_t receiveFilterQueue;//定义了过滤器的一个队列
...
**/
@interface GCDAsyncUdpSendPacket : NSObject {   //这里又定义了一个类GCDAsyncUdpSendPacket
@public
	NSData *buffer;
...
@interface GCDAsyncUdpSpecialPacket : NSObject { //这里又定义了一个类GCDAsyncUdpSpecialPacket
@public
//	uint8_t type;
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq   //我初始化用的是这句
{
	LogTrace();
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	...
	if ((self = [super init]))
	{
		...
		if (dq)
		{
			...
			dispatch_retain(delegateQueue);//增加引用计数
			#endif
		}
		...
		socket4FD = SOCKET_NULL;//初始化为-1
		socket6FD = SOCKET_NULL;
		
		if (sq)
		{
            //NSAssert判断是否出错，如果出错则抛出异常
			...
			NSAssert(sq != dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),
			         @"The given socketQueue parameter must not be a concurrent queue.");
			//这句话翻译下来就是：被给的队列的参数不能够是当前的队列！必须是全局并发的队列，只能够在这三个参数中选择
			socketQueue = sq;
		...
		{
			socketQueue = dispatch_queue_create([GCDAsyncUdpSocketQueueName UTF8String], NULL);//这里需要转换？socketQueue是串行队列？
		}
		...
		IsOnSocketQueueOrTargetQueueKey = &IsOnSocketQueueOrTargetQueueKey;//这里竟然用地址等于地址的地址，英文有解释的

		void *nonNullUnusedPointer = (__bridge void *)self;
		dispatch_queue_set_specific(socketQueue, IsOnSocketQueueOrTargetQueueKey, nonNullUnusedPointer, NULL);//GCD本身是不可重入的，当设置了这一条的时候，根据IsOnSocketQueueOrTargetQueueKey就可以在下面进行相关操作了。
		
		currentSend = nil;
		sendQueue = [[NSMutableArray alloc] initWithCapacity:5];//初始化为5个对象长的可变数组
		
		#if TARGET_OS_IPHONE
		[[NSNotificationCenter defaultCenter] addObserver:self
		                                         selector:@selector(applicationWillEnterForeground:)
		                                             name:UIApplicationWillEnterForegroundNotification
		                                           object:nil];//观察者注册消息通知
		#endif
	}
	return self;
}
...
- (void)setupSendAndReceiveSourcesForSocket4
{
	...
	send4Source = dispatch_source_create(DISPATCH_SOURCE_TYPE_WRITE, socket4FD, 0, socketQueue);//DISPATCH_SOURCE_TYPE_WRITE:socket操作的读响应
	...
	dispatch_source_set_event_handler(send4Source, ^{ @autoreleasepool {
		...
		LogVerbose(@"dispatch_source_get_data(send4Source) = %lu", dispatch_source_get_data(send4Source));//获取源的数据
...
- (void)suspendSend4Source
{
	if (send4Source && !(flags & kSend4SourceSuspended))
	{
		...
		dispatch_suspend(send4Source);//dispatch_suspend增加计数，挂起队列
		flags |= kSend4SourceSuspended;
	}
}
...
- (BOOL)receiveOnce:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ //dispatch_block_t定义一个block
		...
		if ((flags & kReceiveOnce) == 0)
		{
			...
			flags |=  kReceiveOnce;       // Enable //在这里设置flags的模式，进而在doReceive进行不同模式的接受
...
- (void)doReceive
{
	...
	if (doReceive4)
	{
		...
		result = recvfrom(socket4FD, buf, bufSize, 0, (struct sockaddr *)&sockaddr4, &sockaddr4len);//调用了socket函数
		LogVerbose(@"recvfrom(socket4FD) = %i", (int)result);
		
		if (result > 0)//收到字节数大于0
		{
			...
			}//这里用了realloc，参考http://c.biancheng.net/cpp/html/2536.html
	...
	if (result == 0)//result是前面recv收到的字节数
	{
		waitingForSocket = YES;
	}
	else if (result < 0)
